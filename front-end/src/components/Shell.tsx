...
import {
  LayoutDashboard, Users, FileText, CreditCard, CalendarClock, BarChart3, LogOut, Shield
} from 'lucide-react';
...
export default function Shell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user && user.role !== 'admin') router.replace('/client/dashboard');
  }, [user, loading, router]);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <aside className="w-64 bg-gradient-to-b from-slate-900 to-slate-800 flex flex-col shrink-0 border-r border-slate-700 shadow-lg">
        {/* Logo */}
        <div className="h-16 flex items-center px-6 border-b border-slate-700 bg-gradient-to-r from-blue-600 to-teal-600">
        ...
        <div className="p-4 border-t border-slate-700 bg-slate-700/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-teal-400 flex items-center justify-center flex-shrink-0 shadow-lg">
